...
|---|---|---|---|---|
| 합성 일별 수요 | 2개 타깃, 일 단위, seed 7 | context 256 → horizon 64, 프로모션 11일 | 미래 공변량에 대한 모델 반응을 정답을 알고 검증 | 자체 생성 |
...
## 7.1 S0 합성 known-future 공변량 검증

결정적 합성 수요 2개 타깃, context 256일 → horizon 64일, 프로모션 11일을 불규칙 배치하고 미래 공변량으로 제공했다.[^local-synthetic]

| 방법 | MAE | RMSE | WAPE |
|---|---:|---:|---:|
| 7일 seasonal naive | 7.4471 | 12.7993 | 6.14% |
| TimesFM 3 단변량 | 6.4724 | 11.4290 | 5.34% |
| TimesFM 3 다변량+미래 공변량 | **2.4358** | **3.0796** | **2.01%** |

- 단변량은 naive보다 13.1% 낮았고, 미래 공변량을 준 다변량은 naive보다 67.3%, 단변량보다 62.4% 낮았다.
- 프로모션 반응: 설계된 lift 32.8 / 21.0 대비 예측 lift **32.55 / 21.74**. 미래 프로모션을 제거한 반사실 실행에서 반응이 사라졌다.
...
*그림 1. 합성 수요 2개 타깃의 holdout. 노란 띠가 예정된 프로모션일이며, 미래 공변량을 받은 다변량 모드(파랑)만 프로모션 봉우리를 재현한다. 이 실험은 공변량 인터페이스가 작동한다는 확인이지 실제 성능 증거가 아니다.*
...
| 주장 | 증거 유형 | 강도 |
|---|---|---|
| known-future 공변량 인터페이스가 작동한다 | 합성 단일 holdout, 반사실 | 기능 확인 |
...
- **미래에 이미 아는 변화를 공변량으로 줄 때.** 합성 프로모션과 M5 가격·행사·SNAP은 미래 판매 변화와 직접 연결돼 작지만 같은 방향의 이득을 냈다.
...
- **단일 holdout 결과(S0, S1)**는 1회 관측이며, Beijing 결과는 regime-shift 사례 보고로만 읽어야 한다.
...
uv run python run_experiment.py --device mps                       # S0 합성
...
[^local-synthetic]: 로컬 합성 benchmark 결과 `artifacts/results.json`.
